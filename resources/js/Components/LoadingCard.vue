<template>
  <div class="p-4 w-full">
    <div class="bg-black rounded-lg shadow-lg overflow-hidden transform transition-transform hover:scale-105 flex flex-col h-full">
      <div ref="scrollContainer" class="p-4 flex-grow flex flex-col font-mono overflow-hidden">
        <ul class="max-w-md space-y-2 text-gray-500 list-inside dark:text-gray-400">
          <li v-for="(line, index) in lines" :key="index" class="flex items-center animate-line text-indigo-500" :style="{ opacity: index < currentLine ? 1 : 0, transform: index < currentLine ? 'translateY(0)' : 'translateY(10px)' }">
          ...
          <li class="flex items-center animate-line text-indigo-500" :style="{ opacity: currentLine > lines.length ? 1 : 0, transform: currentLine > lines.length ? 'translateY(0)' : 'translateY(10px)' }">
            <div role="status">
          ...
          <li ref="finalMessage" class="flex items-center animate-line text-indigo-500" :style="{ opacity: currentLine > lines.length + 1 ? 1 : 0, transform: currentLine > lines.length + 1 ? 'translateY(0)' : 'translateY(10px)' }">
            <svg class="w-4 h-4 me-2 text-green-500 dark:text-green-400 shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
              <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z"/>
            </svg>
